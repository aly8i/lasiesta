...
const TableCart=({mapLocation})=> {
  ...
  const createOrder = async (data) => {
    ...
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/orders/find/table`, {jwt});
      if (res.status === 201) {
        deleteCookie('cart');
        dispatch(resetCart());
  ...
  const remProduct = async(product) => {
    await dispatch(removeProduct({id:product._id,size:product.size,extras:product.extras,quantity:product.quantity,price:product.price}));
  }
  const incProduct = async(product) => {
    await dispatch(incrementProduct({id:product._id,size:product.size,extras:product.extras,quantity:product.quantity,price:product.price}));
  }
  const decProduct = async(product) => {
    await dispatch(decrementProduct({id:product._id,size:product.size,extras:product.extras,quantity:product.quantity,price:product.price}));
  }
      ...
      <div className={styles.right}>
        <div className={styles.wrapper}>
          ...
          <div className={styles.totalText}>
            ...
            <>
              <motion.div className={styles.button} whileTap={{ scale: 0.8}} whileHover={{ scale: 1.1}} onClick={() => handleOrdernow()} >ORDER NOW</motion.div>
              <motion.div className={styles.button} whileTap={{ scale: 0.8}} whileHover={{ scale: 1.1}} onClick={async() => handleCheckout()} >CHECKOUT NOW!</motion.div>
            </>
        </div>
      </div>
      {cash && <TableOrderDetail mapLocation={mapLocation} createOrder={createOrder} editOrder={editOrder} isCheckout={isCheckout}/>}
    </div>
